...
GO_MINOR=1.25
# usque go.mod requires a newer toolchain than the plugin's pinned one
USQUE_GO_MINOR=1.26
USQUE_REPO=https://github.com/Diniboy1123/usque.git

mdigest() { docker buildx imagetools inspect "$1" --format '{{.Manifest.Digest}}'; }

# Latest Go patch release + linux tarball sha256s for a minor series.
go_pins() {  # <minor> -> "<version> <sha_amd64> <sha_arm64>"
...
print(v["version"][2:], sha["amd64"], sha["arm64"])' "$1"
}
...
read -r GO_VERSION GO_SHA256_AMD64 GO_SHA256_ARM64 < <(go_pins "$GO_MINOR")
read -r USQUE_GO_VERSION USQUE_GO_SHA256_AMD64 USQUE_GO_SHA256_ARM64 < <(go_pins "$USQUE_GO_MINOR")

# Latest usque release tag + the full commit hash it points at. ls-remote lists
# the plain ref first and the peeled ^{} entry (annotated tags) last, so take
# the last line: the commit for annotated tags, the ref itself for lightweight.
USQUE_VERSION=$(git ls-remote --tags --refs "$USQUE_REPO" 'v*' \
  | awk -F/ '{print $NF}' | sort -V | tail -1)
[ -n "$USQUE_VERSION" ] || { echo "could not resolve usque release tag" >&2; exit 1; }
USQUE_COMMIT=$(git ls-remote "$USQUE_REPO" "refs/tags/${USQUE_VERSION}^{}" "refs/tags/${USQUE_VERSION}" \
  | awk 'END{print $1}')
[ -n "$USQUE_COMMIT" ] || { echo "could not resolve usque commit for ${USQUE_VERSION}" >&2; exit 1; }
...
# usque needs go >= 1.26.3 (go.mod), so its builder stage uses a separate pinned toolchain.
USQUE_GO_VERSION=${USQUE_GO_VERSION}
USQUE_GO_SHA256_AMD64=${USQUE_GO_SHA256_AMD64}
USQUE_GO_SHA256_ARM64=${USQUE_GO_SHA256_ARM64}
# github.com/Diniboy1123/usque release tag + its full commit hash (build is by commit).
USQUE_VERSION=${USQUE_VERSION}
USQUE_COMMIT=${USQUE_COMMIT}
EOF
