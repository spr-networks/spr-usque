...
WORKDIR /code
COPY code/ /code/
RUN --mount=type=tmpfs,target=/root/go/ go build -trimpath -ldflags "-s -w" -o /usque_plugin /code/
...
ENV DEBIAN_FRONTEND=noninteractive
ARG UBUNTU_SNAPSHOT=20260601T000000Z
# Native Linux gateway runtime: iproute2 installs the fail-closed policy table;
# nftables permits only eth0 <-> warp0 forwarding and performs WARP SNAT.
RUN set -eux; \
...
COPY --from=builder /usque_plugin /
COPY --from=builder-usque /usque_bin /usr/bin/usque
COPY --from=builder-usque /usque/LICENSE.md /usr/share/doc/usque/LICENSE.md
